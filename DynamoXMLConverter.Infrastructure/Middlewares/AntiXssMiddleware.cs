...
namespace DynamoXMLConverter.Infrastructure.Middlewares
{
    public class AntiXssMiddleware
    {
        ...
        private readonly IEnumerable<string> _specialEndpoints = new List<string>
        {
            "/api/emailTemplate/update",
            "/api/emailTemplate/checkForCompilationErrors"
        };
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            bool isProcessingImage = context.Request.Headers.ContentType.Any(c => c.Contains("multipart/form-data"));
            bool isProcessingSpecialEndpoint = false;

            foreach (var endpoint in _specialEndpoints)
            {
                if (context.Request.Path.HasValue && context.Request.Path.Value.Equals(endpoint))
                {
                    isProcessingSpecialEndpoint = true;
                }
            }

            bool shouldCheckForXss = isProcessingImage || isProcessingSpecialEndpoint ? false : true;

            try
            {
                if (shouldCheckForXss)
                {
